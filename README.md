...
Metadata Refresh is also done manually. Metadata files are generated by ```generateFiles.ts``` of each contract.
 ...
 Do this for all commands in [nftData Folder](https://github.com/Mahmadabid/invader-with-imx/tree/master/src/nftData). I [tsconfig.json](https://github.com/Mahmadabid/invader-with-imx/blob/master/tsconfig.json) make sure to replace ```"module": "esnext",``` with ```"module": "CommonJS",```
```

...
export const SpaceInvader: React.FC<SpaceInvadersProps> = ({ gameConst, setGameConst, levels }) => {
  ...
  const handleFirstStart = () => {
    setGameLogic((prevGameLogic) => ({
      ...
      IPXUnclaimed: 0,
      timer: gameConst.Level === 1 ? gameConst.timer + 6 : gameConst.Level === 2 ? gameConst.timer - 6 : gameConst.timer - 10,
      win: false,
  ...
  const handleStart = () => {
    setGameLogic((prevGameLogic) => ({
      ...
      Health: gameConst.Health,
      IPXUnclaimed: 0,
      timer: gameConst.Level === 1 ? gameConst.timer + 5 : gameConst.Level === 2 ? gameConst.timer - 6 : gameConst.timer - 7,
      win: false,
      interval: (prevGameLogic.interval + 1)
    }));

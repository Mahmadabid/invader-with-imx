    ...
    if (gameConst.start && !gameLogic.gameover) {
        setEnemies((currentEnemies: ElementPosition[]) => {
            const updatedEnemies = currentEnemies.map((enemy) => {
                ...
                if (
                    Math.random() < (gameConst.Level === 1 ? (0.025 - gameConst.enemyFire) : gameConst.Level === 2 ? (0.032 - gameConst.enemyFire) : gameConst.Level === 3? (0.0345 - gameConst.enemyFire): (0.0365 - gameConst.enemyFire))
                    && enemyCanFire
                ) {
